...
test('should render Subtitle default props correctly', () => {
  ...
  if (styles) {
    ...
    expect(styles['font-family']).toBe('BrandFont-Display');
...
test('should render Subtitle with default styles on mobile correctly', () => {
  ...
  if (styles) {
    ...
    expect(_calcSize(styles['line-height'])).toBe(19.2);
    expect(styles['font-weight']).toBe('400');
    expect(styles['font-family']).toBe('BrandFont-Display');
    expect(styles.color).toBe('rgb(0, 0, 0)');
  }
...
test('should render Subtitle VDS typescale correctly', () => {
  ...
  if (styles) {
    ...
    expect(styles['font-family']).toBe('BrandFont-Text');
    expect(styles.color).toBe('rgb(0, 0, 0)');
  }
});
...
test('should render title VDS typescale correctly', () => {
  ...
  const { getByText, container } = render(
    <Subtitle id="test" aria-labelledby="test">
      {content}
    </Subtitle>
  );
  expect(container.firstChild).toMatchSnapshot();
