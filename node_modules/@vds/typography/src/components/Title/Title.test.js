...
test('should render title default props correctly', () => {
  ...
  if (styles) {
    ...
    expect(_calcSize(styles['line-height'])).toBe(20);
    // expect(styles['font-weight']).toBe('750');
    expect(styles['font-family']).toBe('Verizon-NHG-eDS');
...
test('should render title large correctly', () => {
  ...
  const styles = window.getComputedStyle(container.firstChild)._values;
  if (styles) {
    expect(_calcSize(styles['font-size'])).toBe(32);
    expect(_calcSize(styles['line-height'])).toBe(32);
    expect(styles['font-weight']).toBe('700');
  }
});
...
test('should render title with default styles on mobile correctly', () => {
  ...
  const styles = window.getComputedStyle(container.firstChild)._values;
  if (styles) {
    expect(_calcSize(styles['font-size'])).toBe(16);
    expect(_calcSize(styles['line-height'])).toBe(16);
    expect(styles['font-weight']).toBe('700');
    expect(styles['font-family']).toBe('Verizon-NHG-eDS');
  }
});
